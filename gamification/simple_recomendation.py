...
#     preference = preferences[username]  # достигнутые достижения для выбранного(авторизованного) пользователя
...
#     similar_users = [user for similarity, user in top_similar]  # 15 пользователей с наибольшим сходством Жаккарда. На выходе получаем список пользователей(только пользователей)
#     rank = {} # список коэффициентов сходства по достижениям
#     for similarity, other_user in top_similar: # по схожести и другим пользователям из списка 15 топов проходимся
#         other_preferences = preferences[other_user] # список достигнутых достижений выбранного пользователя из списка 15 топов
#         for post_id in other_preferences: # для id достижения пользователя из other_preferences
#             if post_id not in preference: # проверяем достигнуто ли достижение авторизованным пользователем
...
#     recommendation_topM = recommendations[1] # Достаём одно необходимое рекомендованное достижение
