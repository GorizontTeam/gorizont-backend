...
def get_top_N_similar(username, preferences, N):
    ...
    if user_preference:
        ...
        # print(similar)
        similar.sort(key=lambda tup: tup[0], reverse=True)
        return similar[:N]
...
def get_achievements_from_user_achievements(preferences):
    return preferences.values_list('achievement', flat=True)
...
def give_achievement_recommendations(username):
    ...
    preference = preferences.filter(
        user__username=username)  # достигнутые достижения для выбранного(авторизованного) пользователя
    top_similar = get_top_N_similar(username, preferences,
                                    N)  # 15 пользователей с наибольшим сходством Жаккарда. На выходе получаем список пар сходство-пользователь
    similar_users = [user for snimilarity, user in
                     top_similar]  # 15 пользователей с наибольшим сходством Жаккарда. На выходе получаем список пользователей(только пользователей)
    ...
    for similarity, other_user in top_similar:  # по схожести и другим пользователям из списка 15 топов проходимся
        other_preferences = preferences.filter(
            user=other_user)  # список достигнутых достижений выбранного пользователя из списка 15 топов
        ...
        for post_id in other_user_achievements:  # для id достижения пользователя из other_preferences
            if post_id not in current_user_achievement:  # проверяем достигнуто ли достижение авторизованным пользователем
                rank.setdefault(post_id,
                                0)  # создаём пустую запись для достижения в ранк, если она не была создана раньше
                rank[post_id] += similarity  # прибавляем коэффициент сходства для достижения
    recommendations = [(similarity / occurs_in(post_id, preferences, similar_users), post_id) for post_id, similarity in
                       rank.items()]  # получаем список достижение-коэффициент насколько подходит для всех достижений
...
#
# пересечение = A B D F
# объединение = A B C D E F G N
#     '''
#     set1 = set(['A', 'B', 'C', 'D', 'E', 'F'])
#     set2 = set(['A', 'B', 'D', 'F', 'N', 'G'])
#     intrsct = float(len(set1.intersection(set2)))
#     print('intrsct', intrsct)
#     union = float(len(set1.union(set2)))
#     print('union', union)
